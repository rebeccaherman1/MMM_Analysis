...
NARI = load('data/ts/MMM_data/observations.mat');
NARI = NARI.var(:,1:(end_yr - 1901 + 1),3);
...
indiv_pr = load('data/pr/cmip6_piC_all.mat');
indiv_ts = load('data/ts/MMM_data/cmip6_piC_all.mat');
...
pr = indiv_pr.runs(pr_inc,:); pr = pr(prI,1:dimlim); pr(pr==0)=nan; 
ts = indiv_ts.runs(ts_inc,:,3); ts = ts(tsI,1:dimlim); ts(ts==0)=nan; 
...
fprintf('For piC simulations, %f [%f, %f]\n\n', mean(t_indiv), lo, hi)