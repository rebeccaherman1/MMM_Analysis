...
switch realm
    ...
    otherwise
        ...
        fnames = {names{1}, 'piC'};
        obs_styles = {'-', '-.'};
        aa = load(['data/', variable, '/', scenarios{2}, '_GM.mat']);
        common_models = aa.piC_models;
end
fig5_names_2 = {'c.', 'd.'};
...
for basin = 1:I
    ...
    for i = 1:xn
        ...
        elseif(strcmp(s, 'noGHG'))
            ...
            %TODO HERE select just AA models!!!!!!!!! define common_models
        ...
        if(contains(s, 'piC'))
            ...
            obs_anom_res = obs_var_a(:,:,basin) - (H.MMM(:,1:length(obs_var_a(:,:,basin)),basin) - mean(H.MMM(:,1:length(obs_var_a(:,:,basin)),basin),2));
        elseif(strcmp(s, 'noGHG'))
        ...
        [yl, periods, ~, ~, ps, CI] = plot_all(h_runs, map, scaling, obs_anom_res, obsT, variable, obs_styles{i}); 
        set(gca, 'FontSize', 12); 
        yls(i) = yl(2);
...
function [yl, periods, periodograms, h_CI, ps, CI] = plot_all(m_runs, color_map, scaling, obsv, obsT, variable, obs_style)
    if nargin < 6
        obs_style = '-';
    end
    global PAD
